...
/**
 * 기상청 중기기온조회 API
 * 예보일로부터 4일에서 10일까지의 최저/최고기온정보를 조회
 */
export async function GET(request) {
  try {
    const { searchParams } = new URL(request.url);
    const regId = searchParams.get('regId') || '11B10101'; // 기본값: 서울
    ...
    const tempData = items[0];
    
    // 기온 데이터 정규화
    const temperatureData = {
      regId: tempData.regId,
      tmFc: tmFc,
      temperatures: []
    };
    ...
    for (let day = startDay; day <= 10; day++) {
      const dayTemp = {
        ...
        minTemp: {
          temp: tempData[`taMin${day}`] || null,
          low: tempData[`taMin${day}Low`] || null,
          high: tempData[`taMin${day}High`] || null
        },
        maxTemp: {
          temp: tempData[`taMax${day}`] || null,
          low: tempData[`taMax${day}Low`] || null,
          high: tempData[`taMax${day}High`] || null
        }
      };
      
      temperatureData.temperatures.push(dayTemp);
    }
    ...
    console.error('중기기온조회 API 에러:', error);
    return NextResponse.json(
